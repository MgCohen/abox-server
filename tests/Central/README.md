...
| Add a test-system invariant | rare — add a `[Fact]` + its `[Rule]` to [`../Harness/Tests/`](../Harness/Tests/README.md) (`ABox.Tests.Harness.Tests`) and a matching `### ` header to the harness self-Rulebook (`Rulebook.md`); these guard the taxonomy/Rulebooks/parity, not the product, and the harness's own self-parity holds them in lockstep |
