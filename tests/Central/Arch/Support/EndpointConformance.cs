...
internal static class EndpointConformance
{
    ...
    // impl assembly exporting only its `<F>Module` (ADR 0011 D2), so this one list gates both the endpoint-visibility
    ...
    // dead-route), any accidentally-`public` endpoint or helper (the ADR-0011 wall at assembly granularity), and a
    // missing EndpointsAssembly anchor (the per-assembly public symbol Host hands to FastEndpoints, ADR 0011 Gate-1).
