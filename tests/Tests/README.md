...
| Add a whole new test *type* | rare — only when no existing type fits. Follow [`../Harness/README.md`](../Harness/README.md) § *Standing up a new test type*: create `<Type>/{Rulebook,Tests,Support}/`, fill `template.md` + `rules.md` from the canonical skeleton, register it in `Structure/Support/TestTypes`, add a `[ParityFact]` (`requireAllCited` if complete), write ≥1 Rule. No csproj edit. |
