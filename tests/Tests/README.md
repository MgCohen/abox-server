...
| Add a behavioral rule (Unit/E2E/Wire/Live) | **co-located, not here** — append a `###` block to the feature's `src/<…>/<Owner>/Tests/<Type>/Rulebook/rules.md` + a `[Rule("<header>")]` test beside it (1:N allowed); the harness's own tests police it. Stand up a new feature's suite with **new-feature-tests** |
