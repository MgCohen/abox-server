...
// The rule book: the '### ' headers in Rules/rules.md are the declared architecture rules. The file
// is copied next to the assembly at build time (see csproj). Each header is enforced by exactly one
// [Rule]-tagged test, linked by name — RuleParityTest guards that correspondence.
internal static class RuleBook
{
    ...
    {
        var path = Path.Combine(AppContext.BaseDirectory, "Rules", "rules.md");
        if (!File.Exists(path))
            throw new FileNotFoundException(
                $"Rule book not found at '{path}'. The csproj must copy Rules/** to the output directory.");
