...
>   decoupled slot recognizers). The hand-written nodes are gone. Adding a
...
| Term | Meaning |
|---|---|
| **Snippet** | A reusable code fragment, authored as a **real, compiling C# method** with typed **slots**. The method body *is* the template. |
| **Slot** | A gap in a snippet to be filled at composition time. Three kinds (see §4.2). |
| **`Slot.Of<T>()`** | The marker for a *body* slot — a real call that compiles and is type-checked. |
...
- A **snippet is a real C# method** annotated `[Snippet]`. Its body is genuine,
  compiler-checked code with typed slots. You can author it, rename its variables,
  and get type errors — like any code.
- **Slots come in three kinds**, all visible in the method signature/body, all
  type-checked.
- The snippet's **signature is its contract** — exactly like `Func<>`/`Action<>`:
  the return type says what it produces, the parameters are its value slots.
...
- Recipe nodes share a normalized base — `IExpr<T>` (produces a value) /
  ...
  slot rejects a `string`-producing node at authoring time.
- A **standalone tool** lowers a recipe to a plain `ScriptData.cs` you commit and
  may hand-edit. Roslyn is used to **parse snippet bodies and substitute slots at
...
squiggles all work. The body is the template; the generator substitutes the slots.
...
> into **one model** (annotated methods with slots) instead of two.

### 4.2 The three slot kinds

Slots are encoded so the snippet still compiles and the generator can find them:

| Slot kind | How it's written | Filled by | Generated node field |
|---|---|---|---|
...
- **`@`-prefixed identifier = name slot.** Invisible to the C# compiler (it's a
  valid identifier, so the snippet compiles), meaningful to our generator. Used
  even when no keyword-escape is needed — `@` *is* our "this is a slot" sigil.
- **by-value param = value slot**, **`ref` param = existing-name slot.** A `ref`
...
- **`Slot.Of<…>()` = body slot.** The generic argument tells you (and the agent,
  and the generator) the slot's shape/type.
...
slots. The generator reads this to decide whether the recipe node is an
`IExpr<T>` or an `IStmt`.
...
slots and emits a node implementing the normalized base:
...
`Count` (value slot) → `IExpr<int>`; `@i` (new-name slot) → `string`;
`Slot.Of<Block>()` (body slot) → `Block`. Because the node carries its produced
type, **composition is statically correct by construction**: an `int` slot only
...
4. Substitute slots at the **node level**:
   - name slots → swap the `@`-identifier / `ref`-param name for the recipe string,
   - value slots → swap the param reference for the recursively-rendered child,
   - body slots → replace the `Slot.Of<…>()` call with the rendered child block.
...
|---|---|---|
| **Snippet** | you author a `[Snippet]` method | the fragment is well-formed C#; slots are typed; rename works |
...
| **Two-grain split** (records for trivial leaves, snippets for boilerplate) | Made redundant by `@var`: once declarations can be real snippets, **everything is one model** — annotated methods with slots. Simpler. |
...
- Implement the generator: parse each snippet body, substitute the three slot kinds,
...
- Replace the hand-written recipe nodes with a Roslyn **source generator** that
  reads `[Snippet]` methods and emits `IExpr<T>`/`IStmt` nodes from the slots.
...
2. **Editing a snippet body** (e.g. the loop bound `<` → `<=`) **flows through** to
   ...
   from `LoopNode.I` / `Ref("i")`, not the marker. The `@i` marker is the slot's
   identity, bound by convention to the node field `I`; in Step 2 the field is
   generated from the marker, so they always agree.)*
3. A deliberately wrong recipe (a `string` producer into an `int` slot) **fails to
...
6. **Snippet base class vs attribute** — revisit `[Snippet]` attribute vs a base
   class/interface. *Test:* does inheritance buy anything now that slots are
