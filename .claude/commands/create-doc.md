...
1. **Doc type** — if the first token names a known doc type, use it. (List them
   with `cd tools/doc-engine && dotnet run --project . -- catalog`.) Otherwise the
   type is **inferred** from the dump.
...
and follow its procedure against the engine in `tools/doc-engine/`: choose the doc
type, author blocks from the catalog, write the instance with a `---` front-matter
block, run `validate` until PASS, then `outline --write`, then grade with the judge.

Report: the doc type (named or inferred), the blocks chosen (one line each), the
final `validate` result, and anything from the dump you deliberately dropped.
