...
- **(a) SSE `Watch`** — the one non-trivial port ([ADR 0009](../../design/adr/0009-fastendpoints-http-boundary.md)
...
- **Validation stays provisional + inline.** Keep the `AddError`→`Send.ErrorsAsync(400)`
  ...
  only; validation-as-a-Step (R-SPINE) is deferred ([ADR 0009](../../design/adr/0009-fastendpoints-http-boundary.md)).
