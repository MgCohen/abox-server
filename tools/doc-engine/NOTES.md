...
| kinds (meta-model) | `block`/`doctype` are not special-cased — each is a `kinds/*.yaml` declaring `fields` + `constraints`; one self-describing `_schema/kind.schema.yaml` is the floor. Adding a kind = adding data. *Instance composition* landed as a block `composes: [child]` field (one nested level, `### member` → `#### child`) once `step` became its real consumer — see ADR 0016; deeper trees stay deferred until a consumer needs them |
