...
{
    [Rule("HookDispatcher runs only the matching react hooks, feeding the event on stdin")]
    [Fact]
    public async Task Runs_matching_react_hooks_with_event_on_stdin()
    {
        ...
        try
        {
            var matching = new HookManifest(
                ...
                new HookWhen(null, "**/docs/**", null), HookMode.React, "cat > captured.json");
            var wrongCwd = new HookManifest(
                Path.Combine(dir, "n.hook"), [HookKind.TurnEnded],
                new HookWhen(null, "**/src/**", null), HookMode.React, "cat > nope.json");
