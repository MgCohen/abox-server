...
1. **Get the code** (latest `main` has the merged refactor + L9 substrate):
   ...
   dotnet build ABox.slnx     # expect: warning-free
   ```
...
2. **Un-skip the live smoke tests.** They are `[Fact(Skip = "integration: …")]` by
   ...
   - `tests/ABox.Tests/ClaudeSmokeTests.cs` → `FlowLauncher_drives_the_registered_claude_flow_end_to_end`
   - *(optional)* `tests/ABox.Tests/CodexSmokeTests.cs`
...
3. **Run just those tests** and watch the output:
   ```
   dotnet test ABox.slnx --filter "FullyQualifiedName~SmokeTests"
   ```
