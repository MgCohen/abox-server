...
- **Labels read as bare lead-ins, not just bullets.** A `- **Label:**` bullet and a bare
  ...
  its `Outcome` as a closing paragraph instead of bullets that a neighbouring `#####` heading visually
  outranks. The bare form counts **only when its name is declared**, so ordinary bold-lead prose
  (`**Note:** …`) stays prose; the bullet form remains the closed set.
- **Nested blocks render two heading levels deeper, and a `---` separates procedures.** A composed child
  is parsed at `#####` (h5), not `####` — two levels below its `###` procedure, one level skipped — so a
  step reads as visibly subordinate to its procedure in raw markdown rather than near-equal weight. A
  single `---` rule sits **between** procedures (not before each Outcome), marking the independent-menu
  boundaries; the last procedure carries no trailing rule.
