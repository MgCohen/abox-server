...
internal static class Program
{
    ...
    private static string ResolveRoot(string? explicitRoot)
    {
        ...
        // In-repo use resolves the catalog by walking up from the CWD; an INSTALLED tool has no repo catalog on
        // that path, so fall back to the tool's own install dir, where the catalog ships as packed content.
        return WalkUpForCatalog(Directory.GetCurrentDirectory())
            ?? WalkUpForCatalog(AppContext.BaseDirectory)
            ?? Directory.GetCurrentDirectory();
    }

    private static string? WalkUpForCatalog(string start)
    {
        for (var dir = new DirectoryInfo(start); dir is not null; dir = dir.Parent)
            if (File.Exists(Path.Combine(dir.FullName, "_schema", "kind.schema.yaml")))
                return dir.FullName;
        return null;
    }
