...
public sealed class AgentFactory(ResolverSelector resolvers, AutoPolicy autoPolicy, SandboxSettings claudeBox, CodexSandbox codexBox) : IAgentFactory
{
    public Agent Create(AgentConfig config, string projectDir)
    {
        ...
        {
            CodexConfig codex => new Agent(new CodexProvider(codex, codexBox), resolver, cap, projectDir),
            ClaudeConfig claude => new Agent(new ClaudeProvider(claude, resolver, autoPolicy, claudeBox), resolver, cap, projectDir),
